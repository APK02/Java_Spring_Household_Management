...
public class AuthenticationService {
    ...
    public void romaneasca(User sessionUser, HttpSession session) {
        if (emailUserSession == null) {
            emailUserSession = sessionUser.getEmail();
        }

        if (sessionUser == null || sessionUser.getEmail() == null) {
            sessionUser = repository.findByEmail(emailUserSession);
            if (sessionUser == null || sessionUser.getEmail() == null)
                throw new RuntimeException("User not found in session or user ID is null");
